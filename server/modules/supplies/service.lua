...
local function lineEffect(item)
    ...
    local coverage = math.max(0, math.floor((tonumber(effect.protectionCycleRatio) or 0) * 100 + 0.5))
    ...
    if item.tool and effect.action == 'weed' then
        return ('Removes %d%% weeds%s · %d uses'):format(effect.weedRemoval,
            coverage > 0 and (' · %d%% of crop cycle'):format(coverage) or '', effect.uses)
    end
    ...
    if effect.action == 'fertilize' then
        return ('+%d nutrients · %d%% retention for %d%% of crop cycle'):format(effect.amount, strength, coverage)
    end
    if effect.action == 'treat_pest' then
        return ('-%d pest pressure · %d%% suppression for %d%% of crop cycle'):format(effect.reduction, strength, coverage)
    end
