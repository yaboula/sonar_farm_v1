...
export function LeasesView() {
  ...
  return <DeepViewShell eyebrow="Territorial obligations" title="Leases" subtitle="Land access, recurring costs and planting restrictions share one authoritative record" breadcrumb={["Company", "Leases"]} backLabel="Back to Company" onBack={() => navigate("/company")} status={<StatusPill tone={grace.length ? "danger" : "success"}>{grace.length ? `${grace.length} Grace Period` : `${operated.length} Operated`}</StatusPill>}>
    <div className="lease-summary-strip"><span><strong>{operated.length}</strong><small>Operated Fields</small></span><span><strong>${data.filter((lease) => lease.status === "active").reduce((sum, lease) => sum + lease.recurringPrice, 0).toLocaleString("en-US")}</strong><small>Recurring obligations</small></span><span><strong>{data.reduce((sum, lease) => sum + lease.capacity, 0)}</strong><small>Total listed capacity</small></span>{grace[0] ? <button type="button" onClick={() => navigate(`/company/leases/${grace[0].id}`)}><Warning size={18} /><span><strong>Grace Period</strong><small>{grace[0].graceDeadline}</small></span><ArrowRight size={15} /></button> : null}</div>
    ...
    <DetailCard eyebrow="Land portfolio" title={`${visible.length} Lease records`}><div className="lease-card-grid">{visible.map((lease) => <button type="button" key={lease.id} data-status={lease.status} onClick={() => navigate(`/company/leases/${lease.id}`)}><div className="lease-card-head"><MapPin size={22} /><span><strong>{lease.fieldName}</strong><small>{lease.location}</small></span><em>{lease.status.replace("_", " ")}</em></div><div className="lease-card-facts"><span><b>{lease.capacity}</b><small>Slots</small></span><span><b>{lease.recurringPrice ? `$${lease.recurringPrice.toLocaleString("en-US")}` : "Included"}</b><small>{lease.billing}</small></span></div><p>{lease.activeCrops}</p>{lease.restriction ? <div className="lease-restriction"><Warning size={15} />{lease.restriction}</div> : <div className="lease-access"><Plant size={15} />{lease.nextPayment ? `Next payment · ${lease.nextPayment}` : "Permanent company access"}</div>}<span className="lease-open">Review Lease<ArrowRight size={15} /></span></button>)}</div>{!visible.length ? <div className="inline-empty"><CalendarCheck size={21} />No Leases match this status.</div> : null}</DetailCard>
  </DeepViewShell>;
}
